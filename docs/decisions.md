...
- **配套语义决定（客户确认）**：
  1. **cwd 自动继承**：标准 `sudo` 会继承当前目录，双通道若不处理即为**真退化**。
     **做法：提权前按需探测一次**（往数据面发一条内部帧 `printf '%s' "$PWD"`，
     读到该命令的结束标记即为路径；该探测帧不记入命令历史）。
     **这取代原文的"把 `$PWD` 附加到结束标记"**，理由：
     - 附加到标记要改**协议格式**（解析器 + 全部 e2e 断言 + 文档），而路径可能含
       `__`（正是标记分隔符）甚至换行；又不能依赖 `base64`（目标机不保证安装），
       纯 bash 内建做十六进制编码对**多字节 UTF-8 路径**不可靠；
     - 探测式**零协议格式改动**，按"读到结束标记为止"取回整段输出，**天然容忍**
       `__` 与换行；
     - **更根本的是冷启动缺口**（客户指出）：附加到标记依赖"至少执行过一条命令"，
       若 Agent 的**第一条命令就是提权**，便无从获得当前目录（会话建立时虽会抓一次
       `EnvSnapshot.pwd`，但它可能解析失败、且只在建立时有效）。探测式没有这个依赖。
     代价：每次 `run_as_root` 多一次往返（提权本就要新建通道、`ask` 还需人类批准，可忽略）。
     `cwd` 参数可显式覆盖；**自动 `cd` 失败必须明确报错**（P1），不得静默换目录执行；
