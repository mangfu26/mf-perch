...
- **配套语义决定（客户确认）**：
  1. **cwd 自动继承**：标准 `sudo` 会继承当前目录，双通道若不处理即为**真退化**。
     做法：把 `$PWD` 附加到结束标记（`__MF_PERCH_END__<nonce>__<cmd_id>__<rc>__<pwd编码>`），
     应用据此跟踪数据面当前目录，提权时自动组装 `cd <目录> && …`；
     复用已有的 `EnvSnapshot { pwd, path, bash_version }` 链路（目前只在会话建立时抓一次，
     需补"随命令刷新"）。`cwd` 参数可显式覆盖；**自动 cd 失败必须明确报错**（P1），
     不得静默换目录执行；
