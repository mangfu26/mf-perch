...
| 内容类型 | 排布 | 示例 |
| ---- | ---- | ---- |
| 卡片集合（同类实体的列表） | **响应式网格**：`grid-cols-[repeat(auto-fill,minmax(320px,1fr))]` | SSH 主机、认证信息、终端列表 |
| 日志 / 输出类（需要连续阅读与横向空间） | **单列**，不网格化 | 命令审计条目、终端详情的命令时间线 |

> 判断依据：能"一眼比较多条同类项"的内容用网格；逐条连续阅读的内容用单列。
> 网格的最小列宽取 320px：低于此宽度卡片内的地址、指纹等等宽文本会明显折行。
