...
> **时效性：快照（2026-09-10）**——本文是某一时点的审计报告，不是长期维护的现行规范。
> 报告中的问题**当时已全部修复**并提交；但代码此后可能继续变化。
> 若要做新的审计，请另写一份，不要直接改写本文（保留历史便于对比）。
> 其中的经验教训已沉淀为 [`design/principles.md`](design/principles.md) 的 **P3**
> （"安全属性必须有能区分对错的测试"），并在 [`AGENTS.md`](../AGENTS.md) §2.6 安全红线中引用。
