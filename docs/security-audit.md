...
> **此后变化（2026-09-16 补记；不改动下文历史条目）**
>
> 提权机制已由 **D47 / D48 / D49** 整体重做，本文 **V1** 与 **V14** 描述的
> askpass + FIFO 投递链路（远端写 `$HOME/.mf-perch/sudopw.fifo.*`、经 channel stdin
> 投递密码、会话结束再清理）**已被整体移除**：提权改由应用自建的一条短命通道以
> `sudo -S` 投递，数据面上的 `sudo` 被明确拒绝，**远端不再产生任何本应用的文件**。
> §6"验证汇总"里那次 V1 差分验证所依赖的两条清理时序回归测试，也随机制一并删除
> （记录见 [`AGENTS.md`](../AGENTS.md) §4.6，**不要再去代码里找它们**）。
>
> 因此读 V1 / V14 时请理解为**当时的**机制与修复；当前设计以
> [`decisions.md`](decisions.md) **D47 / D48 / D49** 为准。
> 本文 §4"遗留项"的两条**仍然有效**：V2（协议标记可被 Agent 伪造）与远程明文传输（D30）。
