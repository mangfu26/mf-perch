...
| **sudo 凭据边界（V1）** | ✅ **已根治（D47 / D48 / D49）** | 机制不在此处复述，见 **D47–D49** 与 [`design/sudo.md`](docs/design/sudo.md) §0。对外说辞两条红线：<br>① 只说"提权密码不进入 Agent 的用户域"，**不宣称"绝对不可读"**——同 UID 理论上仍可尝试读提权通道进程的 stdin，可行性取决于 `yama/ptrace_scope`（**未实测**，属如实标注的残余风险）；<br>② `Defaults requiretty` 的主机**不支持提权**（明确报错并给出两条出路），**不得声称全主机可用**；D47 定稿不做 PTY 回退。该限制约束的是 `sudo` 行为，**`not_needed` 档不经 `sudo`、不受此限**（见下条） |
| **提权策略第四档 `not_needed`** | ⚠️ 已实现，**真实 root 登录主机未实测** | 该档由**人类声明**"登录身份本身已是特权用户"，实际 uid 仍由远端核实、非 0 时如实告警。**不得对外声称该路径经过端到端验证**：测试环境要跑它需 sshd 开 `PermitRootLogin`，属需客户授权的动作（现状与理由见 **D60**） |
