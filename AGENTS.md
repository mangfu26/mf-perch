...
| 集成分支   | `develop/init`| **开发主线**（首发代码已于 2026-09-18 并入 `main`） | 各轮开发分支合并到这里；`main` 只在客户指示时从本分支吸收稳定代码——**2026-09-18 客户已指示并入一次**，本分支继续作为后续开发主线保留 |
...
- 稳定分支：`main`（发布基线；发布通过在 `main` 上打 tag 完成）
  - **2026-09-18 已建立**：经客户指示，由 `develop/init` 建立（`git branch main develop/init`），
    两分支当时指向同一提交；`main` 的配置已跟踪 `origin/main`。
  - **尚未推送到远端**，也**未打任何 tag**（推送与打 tag 都需客户单独指示，见 §1.4 / §1.5）。
- 当前开发主线：`develop/init`（首发代码已并入 `main` 后，本分支继续作为后续开发主线保留）
- 远程仓库地址：以本地 `git remote -v` 为准（`git@github.com:mangfu26/mf-perch.git`）
  - **远端当前没有任何分支**（2026-09-18 用只读的 `git ls-remote --heads origin` 核实）——
    首次推送会是普通 push，不需要 `--force`。
