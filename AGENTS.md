...
| 文档 | 内容 | 时效性 |
| ---- | ---- | ---- |
| [`docs/decisions.md`](docs/decisions.md) | **技术决策记录（ADR）索引**：按域索引全部决策 + **取代关系表**（正文在分片，一条一个文件） | 现行 |
| [`docs/decisions/`](docs/decisions/) | ADR **正文分片**，一条一个 `D<编号>.md`；由索引进入，不要整目录通读 | 现行（个别条目标"部分失效"） |
| [`docs/decisions/archive/`](docs/decisions/archive/) | **整条已被移除/推翻**的决策原文（有哪几条、被谁取代：看该目录内文件的快照抬头与索引 §3 取代表） | **快照** |

> `decisions.md`（索引）+ 其指向的分片是**技术决策的唯一事实来源**，其它文档与它冲突时以它为准。
> **`D<编号>` 是跨语言外键**（Rust 注释里大量引用，量级口径见 **D52**）：只增不改号、不重排、不复用；
...
| [`docs/open-questions.md`](docs/open-questions.md) | 需求澄清清单：问题、状态、客户答复（条目到几号以该文件为准） | 现行 |
...
8. **不登记能从被索引对象自身导出的内容**（**D57**，判据由 **D59** 一般化）：现行文档里**不写**
   "别处已有"的副本——统计数字（多少条测试 / 多大规模 / 多少个命令）、**编号区间**
   （`D1–D56`、`Q1–Q36`）、**成员清单**（某目录"现含哪几条"）——一次与本文档无关的提交就会失真，
   留下"看着权威、其实是错的"读数比不写更糟。需要时**现取**（`cargo test -j 2`、
   `ls docs/decisions/`、`git grep -c`）或**指向权威落点**（"条目到几号以该文件为准"）。
   **判据**：这一处的内容，打开它所指的文件、或跑那条命令，能不能自己得到？能 → 不登记。
   **计数词指向别处的清单时删掉**（"共有 N 道门禁"，而清单在下面的表里）：加一行表项不会提醒人回来改数。
   **同一句内自带枚举的计数不受影响**（"查三处：定义 / 注册 / 调用"）——改枚举必然改到这句话。
   **不受本条约束**：快照文档与 ADR 里带日期的实测读数（不追改，删了等于销毁证据）；
   **本身即权威**的清单（§4.4 缺口表、§4.6 债务表、索引 §3 取代表——它们是原始记录，不是副本）。
...
| **sudo 凭据边界（V1）** | ✅ **已根治（D47 / D48 / D49）** | 机制不在此处复述，见 **D47–D49** 与 [`design/sudo.md`](docs/design/sudo.md) §0。对外说辞两条红线：<br>① 只说"提权密码不进入 Agent 的用户域"，**不宣称"绝对不可读"**——同 UID 理论上仍可尝试读提权通道进程的 stdin，可行性取决于 `yama/ptrace_scope`（**未实测**，属如实标注的残余风险）；<br>② `Defaults requiretty` 的主机**不支持提权**（明确报错并给出两条出路），**不得声称全主机可用**；D47 定稿不做 PTY 回退。该限制约束的是 `sudo` 行为，**免 `sudo` 的特权身份路径不受此限**（见下条） |
| **特权身份免提权（D60）** | ⚠️ 已实现，**真实 root 登录主机未实测** | "该身份登录即已是特权用户"由人类**声明在认证信息上**（`credentials.is_privileged`），实际 uid 仍由远端核实、非 0 时如实告警。对外说辞两条红线：<br>① **不得声称该路径经过端到端验证**——测试环境要跑它需 sshd 开 `PermitRootLogin`，属需客户授权的动作（现状与理由见 **D60**）；<br>② **不得声称"标记为 `deny` 就能挡住 Agent 以 root 执行命令"**——特权身份下数据面的普通命令本身就是 root，`deny` 只关掉了 `run_as_root` 这一个入口；命令历史也**尚未**把这类命令标成特权执行（缺口登记为 **Q43**） |
...
# 条数不在文档里维护，见 §4.3.8 与 D57 / D59）。注意 mcp_e2e 里有一条只等空闲超时的慢用例，
# 整组一起跑可能超过单条命令的时限——分组跑更稳）
cd src-tauri && cargo test -j 2 -- --ignored --test-threads=1

# 前端默认门禁（都不需要真实环境，清单与分工见 §5.9）
